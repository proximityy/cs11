...
// Edited for HW0 by: Zachary Meyers (zmeyer02)
// Date: 01/24/23
...
int main() {
  ...
  //Prompt the user for a shift amount
  cout << "Shift amount (between 0 and 25): ";
  //Retrieve the word from the user and store it in shift_amount
  cin >> shift_amount;
  ...
  while ( next_letter_pos <= length_of_word ) {
    ...
    if ( word_to_encrypt[next_letter_pos] <= ('z' - shift_amount) ) {
      //There is no wrap around if the letter being evaluated is lower or equal in the alphabet than 26 minus the shift amount
      new_letter = word_to_encrypt[next_letter_pos] + shift_amount;
    } 
    else {
      //Otherwise, there is a wrap around
